...
{int i=id_start; while(File_Exists((filename_base+Number_To_String(i)+filename_suffix)))++i;if(id_result)*id_result=i;
...
{int id=id_start;while(Directory_Exists((directory_name_base+Number_To_String(id))))++id; 
if(id_final)*id_final=id;return directory_name_base+Number_To_String(id);}
...
if(!Create_Directory(output_directory,false)){std::cerr << "Can't create directory " << output_directory << std::endl;exit(1);} 
else std::cout << "Created directory " << output_directory << std::endl;return output_directory;}
//###################################################################
...
{d=T::Create();Read_From_File<RW>(filename,*d);}
//#####################################################################
}
}
#endif
