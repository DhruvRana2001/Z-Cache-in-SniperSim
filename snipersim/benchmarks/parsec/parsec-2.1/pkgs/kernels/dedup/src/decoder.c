...
Reassemble(void * args) {
  ...
  if (args != NULL) {
    fd = open((char *)args, O_CREAT|O_WRONLY|O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
    if (fd < 0) 
  ...
  while (1) {
    ...
    case TYPE_HEAD:
      ...
      if (fd == -1) {
        fd = open(head->filename, O_CREAT|O_WRONLY|O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
        if (fd < 0)
          perror("head_open");
      }
...
RecvBlock(void * args) 
{
  ...
  fd = open(conf->infile, O_RDONLY|O_LARGEFILE, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
  if (fd < 0) {
    perror("infile open");
    return NULL;
  }
