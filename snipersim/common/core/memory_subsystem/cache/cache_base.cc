...
CacheBase::parseAddressHash(String hash_name)
{
   ...
   else if (hash_name == "skew")
      return CacheBase::HASH_SKEW;
   else if (hash_name == "z")
      return CacheBase::HASH_Z;

   else
      LOG_PRINT_ERROR("Invalid address hash function %s", hash_name.c_str());
}

// Tomwi: Idea to return the index in the first way? It won't help much though
// as the full address is need to determine where the other blocks in the other
// ways reside.
void
CacheBase::splitAddress(const IntPtr addr, IntPtr& tag, UInt32& set_index) const
{
   ...
   switch(m_hash)
   {
      // Tomwi: splitting the address into amongs other _one_ set index
      // has no meaning for skew/zcaches. Just make sure we don't error out
      case CacheBase::HASH_Z:
      case CacheBase::HASH_SKEW:
