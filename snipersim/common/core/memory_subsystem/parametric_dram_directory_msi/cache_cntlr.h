...
namespace ParametricDramDirectoryMSI
{
   ...
   class vantageState {
	public:
		UInt32 CurrentTs, AccessCounter, ActualSize, TargetSize, SetPointTs, CandsSeen,
		CandsDemoted;
	private:
   };
   ...
   class CacheMasterCntlr
   {
      private:
         ...
         std::vector<SetLock> m_hash_setlocks;
	 vantageState* vantageStates;
	 Lock zreplacement;
         ...
         SetLock* getSetLock(IntPtr addr, int way);
         SetLock* getSetLock2(IntPtr addr, int way);
         ...
         //int skewHashIdx(IntPtr addr, int way);
	 Lock* getZLock(void){ return &zreplacement;}
   ...
   class CacheCntlr : ::CacheCntlr
   {
	...
	bool skew;
         ...
         void retrieveCacheBlock(IntPtr address, Byte* data_buf, ShmemPerfModel::Thread_t thread_num, bool update_replacement, Core::ExtraMemoryRequestInfo *info, SharedCacheBlockInfo** blockinfo = NULL);
