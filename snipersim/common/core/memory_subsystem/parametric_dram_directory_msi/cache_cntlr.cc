...
#  define MYLOG(...) printf(__VA_ARGS__); printf("\n");
...
char CStateString(CacheState::cstate_t cstate) {
    //printf("entering Cstatestring with %d\n", cstate);
    switch(cstate)
    {
...
void CacheMasterCntlr::createSetLocks(UInt32 cache_block_size, UInt32 num_sets, UInt32 core_offset, UInt32 num_cores)
{
    ...
    m_num_sets = num_sets;
    if(m_cache->getHashType() == CacheBase::HASH_Z || m_cache->getHashType() == CacheBase::HASH_SKEW) {
        printf("Creating %d x %d hash-locks\n", m_cache->getNumSets(), m_cache->getAssociativity());
        m_hash_setlocks.resize(m_cache->getNumSets()*m_cache->getAssociativity(), SetLock(core_offset, num_cores));
    }
    m_setlocks.resize(m_num_sets, SetLock(core_offset, num_cores));

    // Also create our partition states for vantage here
    vantageStates = new vantageState[num_cores+1];
    for(UInt32 i=0; i<num_cores+1; i++) {
        vantageStates[i] = vantageState();
    }
}
...
SetLock*
CacheMasterCntlr::getSetLock(IntPtr addr, int way)

{

    UInt32 LLC_num_sets = m_cache->getNumSets();
    if(m_cache->getHashType() == CacheBase::HASH_SKEW) {
        IntPtr idx = CacheSetSkew::hashedIndex(addr, way, LLC_num_sets, m_log_blocksize);
        return &m_hash_setlocks.at(idx + way * LLC_num_sets);
    }
    else if(m_cache->getHashType() == CacheBase::HASH_Z) {
        IntPtr idx = CacheSetZ::hashedIndex(addr>>m_log_blocksize, way, m_log_blocksize);
    ...
    m_shmem_perf_model(shmem_perf_model)
{
    ...
    if (isMasterCache())
    {
        ...
        LOG_ASSERT_ERROR(!Sim()->getCfg()->getBoolDefault("perf_model/" + cache_params.configName + "/atd/enabled", false), "ATD not supported");
    ...
    Core::ExtraMemoryRequestInfo *info)
{
    ...
    if (cache_hit)
    {
        ...
        if(!operationPermissibleinCache(ca_address, mem_op_type)) {
            fprintf(stdout, "b0rk on core %d %lx %d\n", m_core_id, ca_address, mem_op_type);
            fprintf(stdout, "cache block info gives: %p\n", getCacheBlockInfo(ca_address));

        }
        LOG_ASSERT_ERROR(operationPermissibleinCache(ca_address, mem_op_type),
                         "Expected %x to be valid in L1, mem_op_type was %x", ca_address, mem_op_type);
...
CacheCntlr::copyDataFromNextLevel(Core::mem_op_t mem_op_type, IntPtr address, bool modeled, SubsecondTime t_now, Core::ExtraMemoryRequestInfo *info)
{
    ...
    if(!m_next_cache_cntlr->operationPermissibleinCache(address, mem_op_type)) {
        fprintf(stdout, "Core %d tried to read %lx %lx\n", m_core_id, address, address>>6);
    }
    ...
    else
    {
        ...
        assert(!isLastLevel()); // Is not allowed with a zcache
...
CacheCntlr::processShmemReqFromPrevCache(CacheCntlr* requester, Core::mem_op_t mem_op_type, IntPtr address, bool modeled, bool count, Prefetch::prefetch_type_t isPrefetch, SubsecondTime t_issue, bool have_write_lock, Core::ExtraMemoryRequestInfo *info)
{
    ...
    else // !cache_hit: either data is not here, or operation on data is not permitted
    {
        ...
        else // last-level cache
        {
            ...
            // Tomwi: if we can access the dram controller we're at the LLC.
            else if (m_master->m_dram_cntlr)
            {
                ...
                else
                {
                    ...
                    CacheCntlr* LLC = lastLevelCache();
                    if(LLC->m_master->m_cache->getHashType() == CacheBase::HASH_Z) {
                        releaseStackLock(address);
                        LLC->m_master->getZLock()->acquire();
                    }
...
nastylabel:
    ...
    if (cache_hit)
    {
        ...
        Byte data_buf[getCacheBlockSize()];
        SharedCacheBlockInfo* blockinfo;
        retrieveCacheBlock(address, data_buf, ShmemPerfModel::_USER_THREAD, first_hit && count, info, &blockinfo);
...
CacheCntlr::invalidateCacheBlock(IntPtr address)
{
    ...
    if(old_cstate==CacheState::MODIFIED || old_cstate == CacheState::INVALID) {
        fprintf(stdout, "core %x tried to invalidate %lx %lx %d\n", m_core_id, address, address>>6, old_cstate);
        fprintf(stdout, "was it the lastLevel? %s\n", isLastLevel() ? "true" : "false");
    }
...
void
CacheCntlr::retrieveCacheBlock(IntPtr address, Byte* data_buf, ShmemPerfModel::Thread_t thread_num, bool update_replacement, Core::ExtraMemoryRequestInfo *info, SharedCacheBlockInfo** blockinfo)
{
    ...
    if(!cache_block_info) {
        fprintf(stdout, "tried to retrieve %lx on core %x, address %lx, tag %lx\n", address, m_core_id, address, address >> 6);
        fprintf(stdout, "retrieveCacheBlock was called by LLC? %s\n", isLastLevel()? "true" : "false");
    }
    LOG_ASSERT_ERROR(cache_block_info != NULL, "Expected block to be there but it wasn't");
    if(blockinfo)
        *blockinfo = cache_block_info;
...
CacheCntlr::insertCacheBlock(IntPtr address, CacheState::cstate_t cstate, Byte* data_buf, core_id_t requester, ShmemPerfModel::Thread_t thread_num, Core::ExtraMemoryRequestInfo *info = NULL)
{
    ...
    if(getCacheState(address) != CacheState::INVALID) {
        printf("b0rk b0rk b0rk: %lx core %x\n", address, m_core_id);
    }
    ...
    if(m_master->m_cache->getHashType() == CacheBase::HASH_Z) {
        CacheSetZ* meh = dynamic_cast<CacheSetZ*>(*m_master->m_cache->getSets());
        toAlloc = meh->candidates_per_way*m_master->m_cache->getAssociativity();
if(!meh->vantageEnabled()){
        toAlloc2 = meh->getLevels();
}
else{
        toAlloc2 = toAlloc;
}
    ...
    UInt32 extra_locked[toAlloc];
    UInt32 extra_L1[toAlloc2];
    extra_L1[0] = 0xFFFFFFFF;
    extra_locked[0] = 0xFFFFFFFF;
    m_master->m_cache->insertSingleLine(address, data_buf,
                                        &eviction, &evict_address, &evict_block_info, evict_buf,
                                        getShmemPerfModel()->getElapsedTime(thread_num), this, info, &m_master->m_hash_setlocks, &m_master->m_setlocks, extra_locked, extra_L1);
    ...
    if (eviction)
    {
        ...
        if(getCacheState(evict_address)!=CacheState::INVALID) {
            printf("B0rk op core %x, address %x, tag %x\n", m_core_id, address, address>6);
        }
    ...
    //Tomwi: evicted block has been properly updated. Now release potential additional locks.
    for(UInt32 i=0; i<toAlloc; i++) {
        if(extra_locked[i] != 0xFFFFFFFF) {
            m_master->m_hash_setlocks.at(extra_locked[i]).release_exclusive();
        }
    }
    // Now release the set L1/L2 locks
    for(UInt32 i=0; i<toAlloc2; i++) {
        printf("checking L1 lock %d\n", i);
        if(extra_L1[i] != 0xFFFFFFFF) {

            printf("Releasing lock on set %x %d %d\n", extra_L1[i], i, toAlloc2);

            m_master->m_setlocks.at(extra_L1[i]).release_exclusive();
//		printf("unlocked\n");
        }
    }
...
CacheCntlr::updateCacheBlock(IntPtr address, CacheState::cstate_t new_cstate, Transition::reason_t reason, Byte* out_buf, ShmemPerfModel::Thread_t thread_num)
{
    ...
    if (!cache_block_info)
    {
//	printf("We don't have the block %p L1 ? %s %d!!!\n", address, isFirstLevel() ? "true" : "false", m_core_id);
    ...
    LOG_ASSERT_ERROR((getCacheState(address) == CacheState::INVALID) || (getCacheState(address) == new_cstate) || !m_coherent,
                     "state didn't change as we wanted: %c instead of %c for core %d at address %lx and LLC %s", CStateString(getCacheState(address)), CStateString(new_cstate), m_core_id, address, isLastLevel()? "true" : "false");
    ...
    LOG_ASSERT_ERROR((current_cstate == CacheState::INVALID) || (current_cstate == new_cstate) || !m_coherent,
                     "state didn't change as we wanted: %c instead of %c for core %d at address %lx and LLC %s", CStateString(current_cstate), CStateString(new_cstate), m_core_id, address, isLastLevel()? "true" : "false");
...
CacheCntlr::writeCacheBlock(IntPtr address, UInt32 offset, Byte* data_buf, UInt32 data_length, ShmemPerfModel::Thread_t thread_num)
{
    ...
    if (m_master->m_evicting_buf && (address == m_master->m_evicting_address)) {
        ...
        if(cache_block_info == NULL) {
            fprintf(stdout, "Core %d was trying to write %lx %lx %s\n", m_core_id, address, address>>6, isLastLevel()? "true" : "false");
        }
        LOG_ASSERT_ERROR(cache_block_info, "writethrough expected a hit at next-level cache but got miss");
        if(cache_block_info->getCState() != CacheState::MODIFIED) {
            printf("op core %d state %d wappie %lx LLC: %s\n", m_core_id, cache_block_info->getCState(), address, isLastLevel()? "true" : "false");
        }
...
CacheCntlr::acquireLock(UInt64 address)
{
    ...
    if(lastLevelCache()->m_master->m_cache->getHashType() == CacheBase::HASH_SKEW || lastLevelCache()->m_master->m_cache->getHashType() == CacheBase::HASH_Z) {
        for(unsigned int i=0; i<lastLevelCache()->m_master->m_cache->getAssociativity(); i++) {
            lastLevelCache()->m_master->getSetLock(address,i)->acquire_shared(m_core_id);
        }
    }
...
CacheCntlr::releaseLock(UInt64 address)
{
    ...
    if(lastLevelCache()->m_master->m_cache->getHashType() == CacheBase:: HASH_SKEW || lastLevelCache()->m_master->m_cache->getHashType() == CacheBase::HASH_Z) {
        for(unsigned int i=0; i<lastLevelCache()->m_master->m_cache->getAssociativity(); i++) {
            lastLevelCache()->m_master->getSetLock(address,i)->release_shared(m_core_id);
        }
    }
...
//    fprintf(stdout, "Staclock for %p at %d\n", address, (address >> lastLevelCache()->m_master->m_log_blocksize) &(lastLevelCache()->m_master->m_num_sets-1));
    ...
    if (this_is_locked) {
        ...
        // Tomwi: but that's not enough when locking series of locks. Then we really need
        // to release all locks before we can make an exclusive lock.
        if(lastLevelCache()->m_master->m_cache->getHashType() == CacheBase::HASH_SKEW || lastLevelCache()->m_master->m_cache->getHashType() == CacheBase::HASH_Z) {
            for(unsigned int i=0; i<lastLevelCache()->m_master->m_cache->getAssociativity(); i++) {
                lastLevelCache()->m_master->getSetLock(address,i)->release_shared(m_core_id);
            }
    ...
    if(lastLevelCache()->m_master->m_cache->getHashType() == CacheBase::HASH_SKEW || lastLevelCache()->m_master->m_cache->getHashType() == CacheBase::HASH_Z) {
        for(unsigned int i=0; i<lastLevelCache()->m_master->m_cache->getAssociativity(); i++) {
            lastLevelCache()->m_master->getSetLock(address,i)->acquire_exclusive();
        }
    }
...
CacheCntlr::releaseStackLock(UInt64 address, bool this_is_locked)
{
    ...
    if (this_is_locked) {
        if(lastLevelCache()->m_master->m_cache->getHashType() == CacheBase::HASH_SKEW || lastLevelCache()->m_master->m_cache->getHashType() == CacheBase::HASH_Z) {
            for(unsigned int i=0; i<lastLevelCache()->m_master->m_cache->getAssociativity(); i++) {
                lastLevelCache()->m_master->getSetLock(address,i)->downgrade(m_core_id);
            }
        }
        ...
        if(lastLevelCache()->m_master->m_cache->getHashType() == CacheBase::HASH_SKEW || lastLevelCache()->m_master->m_cache->getHashType() == CacheBase::HASH_Z) {
            for(unsigned int i=0; i<lastLevelCache()->m_master->m_cache->getAssociativity(); i++) {
                lastLevelCache()->m_master->getSetLock(address,i)->release_exclusive();
            }
        }
