   ...
   m_enabled(false)
{
   ...
   if (m_core_id_master == getCore()->getId())
   {
      ...
      for(core_id_t core_id = 0; core_id < (core_id_t)Sim()->getConfig()->getApplicationCores(); ++core_id)
      {
	    ...
	    printf("cachesize not matching, setting num_sets to 1\n");
            num_sets = 1;
	 }
         if (Sim()->getCfg()->getStringArray("perf_model/l1_dcache/address_hash", core_id) != "mask" && Sim()->getCfg()->getStringArray("perf_model/l1_dcache/address_hash", core_id) != "skew" &&
Sim()->getCfg()->getStringArray("perf_model/l1_dcache/address_hash", core_id) != "z"){
           printf("Don't know this hash function, setting num_sets to 1\n"); 
	   num_sets = 1;
	 }
         // FIXME: We really should check all cache levels
      }
//      printf("L1 cache has %d sets\n Its size is %d and associativity %d\nSize of an adress is %d bytes", num_sets, cache_parameters[MemComponent::L1_DCACHE].size, cache_parameters[MemComponent::L1_DCACHE].associativity, sizeof(IntPtr));
